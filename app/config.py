...
class Settings(BaseSettings):
    ...
    tailor_abuse_daily_cap: int = 150    # TAILOR_ABUSE_DAILY_CAP — per-user hard ceiling on tailors/day that applies even to "unlimited" plans (and to everyone in pre-revenue mode, when all users ride PRO). Tailoring is user-triggered Claude spend OUTSIDE the scoring lane's daily budget; without this, one scripted user could fire thousands of paid calls a day. 150 is far beyond real human use. 0 disables.
