...
class Settings(BaseSettings):
    ...
    prescore_budget_multiplier: int = 2   # PRESCORE_BUDGET_MULTIPLIER — bounds the ANTHROPIC Tier-1 fallback only: a user's daily allowance = PLAN_LIMITS["finals_daily"] × this (PRO: 250 × 2 = 500, ~$0.93/day, and only on the days OpenAI is down). A RATIO because it is funnel width — every final needs a prescore first, so a fixed number would throttle Tier-1 the moment the ceiling moved. 2 keeps twice the ceiling available, which is the widest funnel a day can use. Lowered 10 -> 2 when finals_daily went 50 -> 250: an Anthropic prescore charges the SAME platform counter as a final (reranker._prescore_anthropic -> _register_final_call), so at ×10 six Pro users on the fallback path would saturate LLM_DAILY_FINAL_CAP and stop finals platform-wide — during an OpenAI outage, which is the one scenario this path exists for. ×2 keeps the pre-change headroom (500/user, as 50 × 10 was). OpenAI prescores are NOT counted — at $0.0002 each a user's entire Tier-1 volume is ~$0.65/month, so capping them only converts an OpenAI outage into a feed outage. The Anthropic path is ~$0.00185 (9x) and DOES need a bound. Prescores were previously charged AS finals whenever they ran on Anthropic, which spent a PRO user's whole day 15 minutes after 00:00 UTC. 0 = unbounded per user (platform caps still apply). OpenAI prescores are NOT counted — at $0.0002 each a user's entire Tier-1 volume is ~$0.65/month, so capping them only converts an OpenAI outage into a feed outage. The Anthropic path is ~$0.00185 (9x) and DOES need a bound. Prescores were previously charged AS finals whenever they ran on Anthropic, which spent a PRO user's whole day 15 minutes after 00:00 UTC. 0 = unbounded per user (platform caps still apply).
    # ── Finals budget (app/matching/finals_budget.py) ────────────────────────
    # The budget aims at a DELIVERY target, not a call count: scoring runs at
    # full speed until PLAN_LIMITS["shortlist_daily"] jobs have reached the
    # board, then stops. PLAN_LIMITS["finals_daily"] is the cost ceiling behind
    # that, and the two settings below stop a day whose Tier-1 has gone wrong
    # long before the ceiling does. No window here is longer than one day.
    finals_yield_window: int = 50           # FINALS_YIELD_WINDOW — MINIMUM finals scored today before the yield stop may return a verdict at all. A SAMPLE SIZE, not a sliding window: at a true hit rate near 10%, zero hits in 10 finals happens 35% of the time (0.9^10) and in 50 finals 0.5% (0.9^50), so a window of 10 fires on a third of healthy users. Raised 10 -> 50 with the stop moved off an in-process ring onto the day ledger; a ring the stop itself prevented refilling made the guard absorbing until the next deploy.
    finals_yield_continue_rate: float = 0.02  # FINALS_YIELD_CONTINUE_RATE — scoring continues while >= this share of TODAY's finals cleared the shortlist bar. Must sit WELL BELOW the true hit rate or the guard is a coin flip rather than a fault detector: of 57,309 real Claude finals 11.6% cleared 65 and fewer clear 70, so the true rate is ~8-10% and anything near it fires on healthy days (production 2026-09-02 stopped on "yield 10% below 20%"; 0.10 was still barely under the true rate). 0.02 with a window of 50 = one hit in fifty keeps it open, zero hits stops the spend, and the two stay in lockstep at rate <= 1/window.
    ...
    llm_daily_final_cap: int = 15000   # LLM_DAILY_FINAL_CAP — PLATFORM runaway backstop across every user, never the allocation. Must clear plan ceiling x expected users or it silently becomes the allocation again: 250 (Pro ceiling) x 50 users = 12,500. Raised 5000 -> 15000 when the per-user budget stopped being a paced 50/day and became a 250/day ceiling. Bounds the platform at ~$37/day at the MEASURED $0.0025/final (2026-09-05, from the reranker's token telemetry over 250 production calls at Haiku 4.5 list rates — the $0.010 in spend.py was 4.1x high and is corrected). NOTE: Anthropic Tier-1 prescores charge this same counter (reranker._prescore_anthropic), which is why prescore_budget_multiplier is 2 and not 10.
    llm_hourly_final_cap: int = 2000   # LLM_HOURLY_FINAL_CAP — smooths a stampede without pacing individual users. 400 was sized for a paced 1.77 finals/hour/user; with scoring now running flat out until the day's target is delivered, 400 would have become the pacer that was just removed. 2000/hour still stops a runaway inside one hour, and keeps >=7 clock hours between a standing start and the daily backstop.
    ...
    stripe_secret_key: str = ""           # STRIPE_SECRET_KEY — sk_test_… first (whole flow, nobody charged), sk_live_… at go-live. ONLY sk_live_ is revenue: billing.stripe_live_mode()/is_paid_entitlement() treat a test-mode subscription as a rehearsal — it earns PRO limits but is never MRR and never overrides the dormancy gate. Production ran on test keys for the whole beta and its one sandbox subscription was reported as $100 MRR.
    ...
    geo_verify_llm_daily_cap: int = 400        # GEO_VERIFY_LLM_DAILY_CAP — platform-wide calls/day, PERSISTED in platform_counter and reserved per call (app/common/daily_counter.py), so it holds across restarts and replicas. 0 = NO calls (a spend control never reads 0 as unlimited). A COUNT, not a dollar figure: what it costs depends on the model that serves the calls — read the ledger (kind=geo_verify), never estimate from here.
    ...
    shortlist_score_threshold: int = 70  # SHORTLIST_SCORE_THRESHOLD — min LLM rerank score (0-100) to shortlist. History: 35 -> 60 (of 57,309 real Claude finals, 44.5% cleared 35 but only 11.6% cleared 65, so the 35-64 tail was ~1,800 jobs/user nobody applied to) -> 70. 70 is a QUALIFIED bar, not a volume knob: with a handful of users there is no sample to calibrate a finer cut on, and a shortlist the user trusts beats a long one they scroll past. Everything clearing it is shown — there is deliberately NO per-day count cap layered on top (daily_shortlist_limit/shortlist_render_cap are 200, far above what a 70 bar produces). Never lower this to fill volume. This ALSO clamps the Tier-1 advance gate (see prescore_advance_threshold), and shortlist_strong_threshold must move with it or the board hides its own shortlist.
    shortlist_strong_threshold: int = 70 # SHORTLIST_STRONG_THRESHOLD — default min-score filter on the board. Held EQUAL to shortlist_score_threshold: at a 70 qualified bar every shortlisted job is by definition a strong fit, so a stricter board default would hide rows we just paid to score. Must never be lower than the shortlist bar (test_settings_defaults).
    ...
    analytics_salt: str = ""               # ANALYTICS_SALT — salts the user key in journey analytics (app/analytics/journey.py); set a secret value in production so keys cannot be recomputed from a user id.
    research_log_sync: bool = False        # tests only: write contact-research observations inline
    internal_user_ids: str = ""            # INTERNAL_USER_IDS — comma-separated account ids (staff/test) excluded from engagement metrics.
    compute_policy_enforced: bool = True  # COMPUTE_POLICY_ENFORCED — app/common/compute_policy.py gates automatic personalised work on the last MEANINGFUL action. 0 = the pre-2026-09-26 behaviour (every user with a résumé is worked for). Kill switch only.
    trial_idle_after_hours: int = 24     # TRIAL_IDLE_AFTER_HOURS — app/common/compute_policy.py: no meaningful action for this long = IDLE: no automatic personalised paid AI calls or research (queue/adoption continue, board kept).
    trial_dormant_after_hours: int = 72  # TRIAL_DORMANT_AFTER_HOURS — past this = DORMANT: every personalised background lane skips the user until they act again.
    user_daily_paid_call_cap: int = 2000     # USER_DAILY_PAID_CALL_CAP — atomic per-user backstop on AUTOMATIC paid provider ATTEMPTS per UTC day (retries and fallbacks each count). Above the plan budgets (Pro 250 finals + Tier-1 prescores); it exists so a loop or a second process cannot outrun the ledger. 0 = unlimited.
    platform_daily_paid_call_cap: int = 40000  # PLATFORM_DAILY_PAID_CALL_CAP — the same, platform-wide, persisted in platform_counter (survives restarts and replicas, unlike llm_daily_final_cap). 0 = unlimited.
    dormant_user_grace_days: int = 1     # DORMANT_USER_GRACE_DAYS — was 21 (audit 2026-09-25, finding 7: a complimentary trial kept its personalized paid search running for three weeks after the person left, and polling alone could keep it alive forever). Users with no MEANINGFUL action (server._is_meaningful_request: a write, opening/downloading a document, starting a fill, or navigating to the dashboard — never a poll, a notification fetch or a token refresh) for this many days are skipped by adoption/matching/scoring/alerts (their pool stops refilling, so no LLM money is spent on them); the next visit re-activates them within one lane tick. Profiles that predate activity tracking (last_active_at NULL) are grandfathered as active. Only a PAID search (billing.is_paid_entitlement: a manual activation, or a Stripe row under sk_live_) overrides the gate — a grandfathered PRO with no subscription row and a test-mode subscription are complimentary and go dormant like anyone else; reading "plan != FREE" as "paying" is how 10 dormant accounts took 90.8% of a week's LLM spend. 0 disables the gate.
    ...
    pulse_max_boards_per_tick: int = 300   # PULSE_MAX_BOARDS_PER_TICK — hard cap per tick, and THE capacity lever for the hourly-floor promise. Sizing arithmetic (do it whenever live_boards grows): demand/hour ≈ live_boards × (60/floor_interval_minutes) + fast_boards × (60/fast_interval_minutes); capacity/hour ≈ this × (3600/tick_seconds), cut further whenever ticks overrun toward pulse_tick_max_seconds. The "~150 boards/min" note this comment used to carry dated from a ~9k-board registry; at 21,505 live boards the floor alone demands ~358 boards/min against a real capacity of ~120-300/min — which is how production reached 18,773 boards (87%) past the floor while the UI said "catching up". The dashboard now reports floor_holding/overdue_pct honestly; raising this (network fetches only, no LLM cost — but watch container CPU/RSS per docs/MEMORY.md) or lengthening the floor for low-yield boards are the two ways to make the promise true again. With PULSE_ADAPTIVE_ENABLED this is the CEILING the adaptive cap grows back toward, not the number every tick selects.
    # ── Adaptive board cap (AIMD) ─────────────────────────────────────────────
    # A fixed 300-board selection assumes the tick can consume 300 boards. When
    # it cannot, the surplus is not free: every selected-then-deferred board
    # was still FETCHED (an HTTP request per board, ~230 wasted per tick at the
    # measured 231/300 deferral) and the ones whose fetch landed still paid
    # their DB writes before the deadline cut the consumer off. Production
    # 2026-09-14..16: deferred boards rose from 1-4K/day to 20,367 / 23,753 /
    # 40,950; ticks/day fell 1,100 → 537; consumer deadline hits went from
    # 2-17/day to 119-241/day; the latest tick selected 300 and deferred 231; a
    # tick took 159s against a 60s interval; upsert_shared p50 1,505ms, p90
    # 38s; 58% of active boards (28,293) were past next_poll_at with a p50 of
    # 1.9h overdue — on a t3a.micro Supabase whose Disk IO budget was nearly
    # exhausted. Selecting more than the lane can finish makes the DB slower,
    # which makes the lane finish less: a feedback loop, so the cap has to
    ...
    # is logged once. Deferral semantics are unchanged: a deferred board is
    # still never recorded as polled (tests/test_pulse_deferral.py).
    pulse_adaptive_enabled: bool = True    # PULSE_ADAPTIVE_ENABLED — 0 restores the constant pulse_max_boards_per_tick selection
    pulse_min_boards_per_tick: int = 40    # PULSE_MIN_BOARDS_PER_TICK — the floor the cap halves down to; ~the number the saturated tick was actually finishing (300 selected, 231 deferred → 69 consumed), so the lane keeps polling under pressure instead of starving the watchlist
    pulse_adaptive_defer_pct: float = 0.5  # PULSE_ADAPTIVE_DEFER_PCT — deferred/selected above this halves the cap. 0.5, not the dashboard's 20% "capacity_limited" marker: the controller reacts to a tick that mostly failed, the dashboard flags one that partly did
    ...
    # ── Tailoring determinism + quality gates ────────────────────────────────
    # A measured audit (2 real résumés, 51 metered Haiku calls) found NO
    # temperature set anywhere in the tailoring stack, so every call ran at the
    # SDK default of 1.0. Two runs of the same résumé against the same JD
    # produced different safety outcomes: one invented "GPU clusters" and paid
    # for a rebuild, the other did not. A fact-check in particular must not be
    # sampled — the same claim against the same evidence has one right answer.
    tailoring_temperature: float = 0.2   # TAILORING_TEMPERATURE — résumé/cover generation. Low, not 0: some variation in phrasing is what keeps the output from reading templated (the anti-fingerprint goal), but 1.0 is what made the honesty guarantee a coin flip.
    verifier_temperature: float = 0.0    # VERIFIER_TEMPERATURE — grounding fact-check and any other verifier call. Zero, always: a sampled verdict means the same résumé passes one run and fails the next.
    doctor_verdict_max_tokens: int = 220 # DOCTOR_VERDICT_MAX_TOKENS — was 120, which truncated the recruiter verdict mid-sentence (stop_reason=max_tokens) on a two-sentence answer the user reads verbatim.
    doctor_min_human_score: int = 55     # DOCTOR_MIN_HUMAN_SCORE — the anti-fingerprint floor. The audit measured tailored résumés scoring 24-44/100 on our OWN "reads human" check that shipped reported as PASS, because the fingerprint penalty was folded into the same number as ATS coverage and cleared 65 anyway. Now a separate gate: below this the draft is rebuilt with the tells named, and if the rebuild still fails, the résumé is delivered but NOT reported as passing (style is not a fabrication, so it never blocks at ERROR). 55 ≈ at most one minor tell.
    tailor_skip_coverage_pct: float = 0.80  # TAILOR_SKIP_COVERAGE_PCT — when the master résumé already matches at least this share of the JD's extracted phrases, skip the rewrite (L0) and ship the master. A full regeneration that cannot add anything honest is pure cost plus a fresh chance to fabricate: the audit measured one pairing at 72% master coverage where tailoring produced +0 phrases for $0.066 and a grounding failure. 0 disables the skip.
    grounding_verify_batch_max: int = 12 # GROUNDING_VERIFY_BATCH_MAX — max changed spans packed into ONE verifier request. Verification used to be one call per bullet, each re-sending the entire master résumé (1,423 input tokens for a 5-token answer); that was 64% of a tailor's bill. Spans are batched with only their own matched source evidence attached.
    join_page_size: int = 0              # JOIN_PAGE_SIZE — page size for join.com's public jobs API. 0 = discover it: the scraper walks a descending ladder on a 422 and memoizes the first size the API accepts (app/discovery/join.py). Production measured EVERY non-throttled join request returning 422 at pageSize=100, which is why every one of ~23.5K join boards reported "0 jobs". Set this once the probe has logged the real limit, then the probe never runs.

    grounding_cache_enabled: bool = True # GROUNDING_CACHE_ENABLED — reuse a verdict when (source evidence, generated text, verifier version) are all identical. There is deliberately NO TTL: the key already covers everything that can change the answer, so an expiry would only re-buy the same verdict.
