...
class Settings(BaseSettings):
    ...
    finals_yield_continue_rate: float = 0.10  # FINALS_YIELD_CONTINUE_RATE — Test B: burst stays open while >= this share of the last N finals cleared the shortlist bar. The guard against a miscalibrated Tier-1 — prescore promising 70s while Claude answers 40s stops the spend even when Test A passes. IN LOCKSTEP with shortlist_score_threshold: of real Claude finals only 11.6% cleared 65 and fewer clear 70, so at 0.20 the burst zone was nominally open and practically never (production 2026-09-02 closed on "yield 10% below 20%"). 0.10 with a window of 10 = one hit in the last ten keeps it open; zero hits still stops the spend.
    finals_pace_enabled: bool = True          # FINALS_PACE_ENABLED — release each day's soft/burst budget along a curve across the UTC day, and the week's along the week, instead of all at 00:00. Production 2026-09-01..03: 100% of both users' finals were spent in the 00:xx UTC hour (176, then 87) and the other 23 hours scored nothing, so everything posted during the US working day (13:00-01:00 UTC) waited for the next midnight; the founder account then hit the weekly ceiling on Wednesday. The budgets' SIZE is unchanged — only when they are available. 0 = the old all-at-midnight behaviour.
    finals_pace_head_start: float = 0.15      # FINALS_PACE_HEAD_START — share of the day's budget available at 00:00 UTC (PRO: 8 of 50 finals), the rest accrues linearly (PRO: ~1.8/hour). A ceiling on cumulative spend, not an hourly quota: a quiet morning's unspent share is still there in the afternoon.
