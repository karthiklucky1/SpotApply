...
class Settings(BaseSettings):
    ...
    prescore_advance_threshold: int = 60  # PRESCORE_ADVANCE_THRESHOLD — Tier-1 fit >= this advances to Claude; below is stamped and drained. The effective gate is min(this, shortlist_score_threshold), so it must be raised IN LOCKSTEP with that bar — leaving it at 35 while the shortlist bar moved to 60 would keep paying Claude to score jobs that can no longer shortlist. Was 30 (advanced ~90% of jobs, a cost ADD), then 35. Keeping the two equal preserves the invariant that anything which could plausibly shortlist still gets the authoritative look.
    ...
    scoring_max_job_age_days: int = 14   # SCORING_MAX_JOB_AGE_DAYS — unscored jobs older than this are bulk-stamped out of the queue (score 8) instead of paying prescores/finals: 'be first to apply' is the product, and a posting that sat unscored for 2+ weeks (outage backlog) is filled/stale. One indexed UPDATE per scoring cycle replaces thousands of LLM calls during a backlog drain. 0 disables.
